...
<NotificationDisplay {themes} />
	...
	<Router>
		<section>
			...
			<aside>
				<nav>
					<div><Link class="listitem" to="/">/</Link></div>

					{#each links as link}
						<div use:tippy={{ content: link.description, placement: "left", theme: "light-border" }}>
							{#if !link.newTarget}
								└─<Link class="listitem" to={link.link}>{link.label}</Link>
							{:else}
								└─<a class="listitem" href={link.link} target="_blank" rel="noopener">{link.label}</a>
							{/if}
						</div>
					{/each}
				</nav>
			...
			<footer>utile.space v{version} &copy; 2021 All rights reserved. Website by <a href="https://sonny.alvesdi.as" title="Sonny Alves Dias">sonny.alvesdi.as</a></footer>
		</section>
	</Router>

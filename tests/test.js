...
test('about page has expected h1', async ({ page }) => {
	...
	await expect(
		page.getByRole('heading', { name: 'Welcome to utile.space' })
	).toBeVisible();
});
...
test('api page has expected h1', async ({ page }) => {
	await page.goto('/api');
	await expect(
		page.getByRole('heading', { name: 'utile.space API Usage License' })
	).toBeVisible();
});
...
test('base64 page works as expected', async ({ page }) => {
	...
	await expect(
		page.getByRole('heading', { name: 'Base64' })
	).toBeVisible();
...
test('dice page works as expected', async ({ page }) => {
	...
	await expect(
		page.getByRole('heading', { name: 'Dice' })
	).toBeVisible();
...
test('int2english page works as expected', async ({ page }) => {
	...
	await expect(
		page.getByRole('heading', { name: 'Int2English' })
	).toBeVisible();
